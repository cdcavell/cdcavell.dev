...
using dis.cdcavell.dev.Models.Account;
...
namespace dis.cdcavell.dev.Controllers
{
    ...
    public class AccountController : ApplicationBaseController<AccountController>
    {
        ...
        /// </summary>
        /// <returns>Task&lt;IActionResult&gt;</returns>
        /// <method>Login(LoginInputModel model, string button)</method>
        [HttpPost("Account/Login")]
        [HttpPost("Account/Login/{id?}")]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Login([Bind(LoginInputModel.BindProperties)] LoginInputModel model, [FromForm(Name = "Button")] string button)
        {
            // Check Username is valid
            if (!string.IsNullOrEmpty(model.Username))
                if (!model.Username.Clean().IsValidEmail())
                    ModelState.AddModelError(nameof(model.Username), _sharedLocalizer["ErrorMessage.Invalid"]);
            ...
            {
                // find user
                var registeredUser = await _userManager.FindByEmailAsync(model.Username);
                if (registeredUser == null)
                {
                    registeredUser = new ApplicationUser
                    {
                        Email = model.Username!.Clean(),
                        UserName = model.Username!.Clean(),
                        NormalizedEmail = model.Username!.Clean().ToUpper(),
                        NormalizedUserName = model.Username!.Clean().ToUpper(),
                        Status = UserStatus.New
                    };

                    TempData["User"] = JsonConvert.SerializeObject(registeredUser);
        ...
        /// </summary>
        /// <returns>Task&lt;IActionResult&gt;</returns>
        /// <method>Logout(string logoutId)</method>
        [HttpGet("Account/Logout")]
        [HttpGet("Account/Logout/{id?}")]
        [AllowAnonymous]
        public async Task<IActionResult> Logout([FromQuery(Name = "logoutId")] string logoutId)
        {
        ...
        /// </summary>
        /// <returns>Task&lt;IActionResult&gt;</returns>
        /// <method>Logout(LogoutInputModel model)</method>
        [HttpPost("Account/Logout")]
        [HttpPost("Account/Logout/{id?}")]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Logout([Bind(LogoutInputModel.BindProperties)] LogoutInputModel model)
        {
        ...
        [HttpGet("Account/AccessDenied/{id?}")]
        [AllowAnonymous]
