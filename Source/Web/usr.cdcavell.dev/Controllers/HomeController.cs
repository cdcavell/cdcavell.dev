...
using System.Net;
using usr.cdcavell.dev.Models.Apis.User;
...
namespace usr.cdcavell.dev.Controllers
{
    ...
    public class HomeController : ApplicationBaseController<HomeController>
    {
        ...
        /// </summary>
        /// <returns>Task&lt;IActionResult&gt;</returns>
        /// <method>AccountInformation()</method>
        [HttpGet("{controller}/AccountInformation")]
        public async Task<IActionResult> AccountInformation()
        {
            ApiClient apiClient = new(_appSettings.Authentication.IdP.Authority);
            apiClient.SetBearerToken((User.Claims?
                .Where(x => x.Type == "BearerToken")
                .Select(x => x.Value)
                .FirstOrDefault() ?? string.Empty)
            );

            UserRolesModel user;
            HttpStatusCode httpStatusCode = await apiClient.SendRequest(HttpMethod.Get, "Api/User/Listing");
            if (apiClient.IsResponseSuccess)
                user = apiClient.GetResponseObject<UserRolesModel>() ?? new();
            else
        ...
        public IActionResult Index()
        {
            ...
            return Redirect(authorityUri + "/Home/Index");
        }
